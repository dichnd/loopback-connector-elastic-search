...
ESConnector.prototype.buildFilter = function (modelName, idName, criteria, size, offset) {
	...
	if (criteria) {
        ...
        if (criteria.where) {
            filter.body.query = filter.body.query || {}
            filter.body.query = Object.assign(filter.body.query, self.buildWhere(modelName, idName, criteria.where).query)
        }
        if(criteria.match) {
            filter.body.query = filter.body.query || {}
            filter.body.query = Object.assign(filter.body.query, self.buildMatch(modelName, idName, criteria.match, true).query)
        }
		if(criteria.score) {
			var function_score = {
