...
module.exports.initialize = function (dataSource, callback) {
    ...
    var connector = dataSource.connector = new ESConnector(settings, dataSource);
    ...
    if (dataSource.constructor.DataAccessObject) {
        ...
        DataAccessObject.search = function (query, callback) {
            return connector.search(query, callback);
        };

        DataAccessObject.get = function (query, callback) {
            return connector.get(query, false, callback);
        };

        DataAccessObject.bulk = function (query, callback) {
            return connector.bulk(query, false, callback);
        };

        DataAccessObject.update = function (query, callback) {
            return connector.update(query, false, callback);
        };

        DataAccessObject.index = function (query, callback) {
            return connector.index(query, false, callback);
        };

        DataAccessObject.delete = function (query, callback) {
            return connector.delete(query, false, callback);
        };

        DataAccessObject.scroll = function (query, callback) {
            return connector.scroll(query, false, callback);
        };

        DataAccessObject.destroy = function (id, callback) {
            return connector.destroy(this.modelName, id, callback);
        };

        DataAccessObject.destroyAll = function (where, callback) {
            return connector.destroyAll(this.modelName, where, callback);
        };

        DataAccessObject.save = function (data, callback) {
            return connector.save(this.modelName, data, callback);
        };

        DataAccessObject.updateAll = function (where, data, options, callback) {
            return connector.updateAll(this.modelName, where, data, options, callback)
        }

        DataAccessObject.updateScript = function (id, data, callback) {
            return connector.updateScript(this.modelName, id, data, callback)
        }

        DataAccessObject.updateOrCreateScript = function (id, data, callback) {
            return connector.updateOrCreateScript(this.modelName, id, data, callback)
        }

        DataAccessObject.updateAllScript = function (where, data, options, callback) {
            return connector.updateAllScript(this.modelName, where, data, options, callback)
        }

        DataAccessObject.count = function (where, callback) {
            return connector.count(this.modelName, where, callback)
        }

        DataAccessObject.find = function (id, callback) {
            return connector.find(this.modelName, id, callback)
        }

        DataAccessObject.all = function (filter, callback) {
            return connector.all(this.modelName, filter, callback)
        }

        DataAccessObject.updateOrCreate = function (data, callback) {
            return connector.updateOrCreate(this.modelName, data, callback)
        }
    }
...
ESConnector.prototype.connect = function (callback) {
    ...
    if (self.db) {
        ...
        self.db = self.elasticClient = new elasticsearch.Client(self.getClientConfig());
...
// ESConnector.prototype.updateMapping = require('./updateMapping.js')({
//     log: log
//     , lodash: _
//     , bluebird: Promise
// });
 ...
 * @return {*|string}
 */
ESConnector.prototype.getIndexFromModel = function (modelName) {
    var self = this
    var indexFromDatasource;
    var mappingFromDatasource = _.find(self.settings.mappings,
        function (mapping) {
            return mapping.name === modelName;
        });
    ...
    return indexFromDatasource || this.searchIndex;
}
 ...
 * @return {*|string}
 */
ESConnector.prototype.getTypeFromModel = function (modelName) {
    var self = this
    var typeFromDatasource;
    var mappingFromDatasource = _.find(self.settings.mappings,
        function (mapping) {
            return mapping.name === modelName;
        });
    ...
    return typeFromDatasource || this.searchType || modelName;
}
...
ESConnector.prototype.buildFilter = function (modelName, idName, criteria, size, offset) {
    ...
    if (criteria) {
        ...
        // }
        if (criteria.where) {
            filter.body.query = filter.body.query || {bool: {}}
            filter.body.query.bool = Object.assign(filter.body.query.bool, self.buildWhere(modelName, idName, criteria.where).query.bool)
        }
        if (criteria.match) {
            filter.body.query = filter.body.query || {bool: {}}
            filter.body.query.bool = Object.assign(filter.body.query.bool, self.buildMatch(modelName, idName, criteria.match, true).query.bool)
        }
        if (criteria.score) {
            var function_score = {
                query: filter.body.query,
                functions: criteria.score
            }
            delete filter.body.query
            filter.body.query = {
...
ESConnector.prototype.buildOrder = function (model, idName, order) {
    ...
    for (var index = 0, len = keys.length; index < len; index++) {
        if (typeof keys[index] == 'object') {
            sort.push(keys[index])
        } else {
            var m = keys[index].match(/\s+(A|DE)SC$/i);
            var key = keys[index];
            key = key.replace(/\s+(A|DE)SC$/i, '').trim();
...
ESConnector.prototype.buildWhere = function (model, idName, where) {
    ...
    self.buildNestedQueries(body, model, idName, where, false);
...
ESConnector.prototype.buildMatch = function (model, idName, where) {
    var self = this;
    log('ESConnector.prototype.buildMatch', 'model', model, 'idName', idName, 'where', JSON.stringify(where, null, 0));
    ...
    self.buildNestedQueries(body, model, idName, where, true);
    if (body && body.query && body.query.bool && body.query.bool.must && body.query.bool.must.length === 0) {
        delete body.query.bool['must']; // jshint ignore:line
    }
    if (body && body.query && body.query.bool && body.query.bool.should && body.query.bool.should.length === 0) {
...
ESConnector.prototype.buildNestedQueries = function (body, model, idName, where, isMatch) {
    /**
    ...
    ESConnector.prototype.buildDeepNestedQueries(true, idName, where, body, rootPath, isMatch);
};

ESConnector.prototype.buildDeepNestedQueries = function (root, idName, where, body, path, isMatch) {
    var self = this;
    _.forEach(where, function (value, key) {
        ...
        if (key === 'and' && Array.isArray(value)) {
            ...
            cond.map(function (c) {
                ...
                self.buildDeepNestedQueries(false, idName, c, body, andPath, isMatch);
            });
        }
            ...
            cond.map(function (c) {
                log('ESConnector.prototype.buildDeepNestedQueries', 'mapped', 'body', JSON.stringify(body, null, 0));
                self.buildDeepNestedQueries(false, idName, c, body, orPath, isMatch);
            });
        } else if (key === '$script') {
            if (root) {
                path.bool.filter = path.bool.filter || {}
                path.bool.filter.script = {
                    script: value
                }
            }
        } else {
            ...
            var reserveSpecs = ['gte', 'gt', 'lte', 'lt', 'between', 'inq', 'nin', 'neq', 'exists']

            if (spec && reserveSpecs.indexOf(spec) != -1) {
                ...
                if (spec == 'exists') {
                    ...
                    if (!cond) {
                        if (root) {
                            path.bool.must_not.push(exists);
                        }
                        else {
                            path.push({bool: {must_not: [exists]}});
                        }
                    } else {
                        if (root) {
                            path.bool.must.push(exists);
                        }
                        else {
                            path.push({bool: {must: [exists]}});
                        }
                    }
                }
                ...
                if (isMatch) {
                    var nestedQuery = value != undefined && {
                            match: {
                                [key]: value
                            }
                        }
                    ...
                    if (Array.isArray(value)) {
                        nestedQuery = {terms: {}};
                        nestedQuery.terms[key] = value
                    } else if (value != undefined) {
                        nestedQuery = {term: {}};
                        nestedQuery.term[key] = value;
                    }
                }
 ...
 */
ESConnector.prototype.buildUpdateScript = function (model, data, callback) {
    var upsert = {}
    var fields = []
    var values = []
    var increaseFields = []
    ...
    _.forEach(data, function (value, key) {
        if (typeof value == 'object') {
            if (value['$add']) {
                increaseFields.push(key)
                increaseValues.push(value['$add'])
                upsert[key] = value['$add']
            } else if (value['$sub']) {
                increaseFields.push(key)
                increaseValues.push(-value['sub'])
                upsert[key] = -value['$sub']
            } else {
                fields.push(key)
                values.push(value)
                upsert[key] = value
        ...
        self.db.search(
            ...
            function (body) {
                ...
                if (filter && filter.include) {
                    ...
                    done(null, result, body.hits.total);
                }
            }, function (err) {
                console.trace(err.message);
                if (err) {
                    return done(err, null, 0);
                }
            }
        );
    }
 ...
 */
ESConnector.prototype.updateAllOld = function updateAll(model, where, data, options, cb) {
    var self = this;
    if (self.debug) {
...
ESConnector.prototype.updateAll = function (model, where, data, options, cb) {
    ...
    var type = this.getTypeFromModel(model)
    var RETRY_ON_CONFLICT = 50;

    var idName = self.idName(model);

    var update = function (page, ids) {
        var bulkBody = []
        _.forEach(ids, function (id) {
            ...
            bulkBody.push({
                doc: data,
                doc_as_upsert: options.doc_as_upsert || undefined
            })
        })
        ...
        self.db.bulk({
            ...
            search(page + 1, update)
        })
    }

    var search = function (page, done) {
        var limit = 100
        var filter = {
            where: where,
            offset: page * limit,
 ...
 * @param {error} cb
 */
ESConnector.prototype.updateAllScript = function (model, where, data, options, cb) {
    if (typeof options == 'function') {
        cb = options
        options = {}
    }
    ...
    var index = this.getIndexFromModel(model)
    var type = this.getTypeFromModel(model)
    var scriptInfo = this.buildUpdateScript(model, data)
    var RETRY_ON_CONFLICT = 50;
        ...
        _.forEach(ids, function (id) {
            ...
            bulkBody.push({
                script: {
                    file: 'update',
                    params: {
                        fields: scriptInfo.fields,
                        values: scriptInfo.values,
                        increaseFields: scriptInfo.increaseFields,
                        increaseValues: scriptInfo.increaseValues
                    }
                },
                upsert: options.upsert ? scriptInfo.upsert : undefined
            })
        })
...
/**
 * add update.groovy to script folder of elasticsearch database to use this method
 * @param model
 * @param id
 * @param data
 *    - use {field: {$add: value}} for increase value
 *    - use {field: {$sub: value}} for subtract value
 * @param done
 */
ESConnector.prototype.updateScript = function (model, id, data, done) {
    var self = this
 ...
 * @param done
 */
ESConnector.prototype.updateOrCreateScript = function (model, id, data, done) {
    var self = this
    var defaults = self.addDefaults(model);
    var scriptInfo = this.buildUpdateScript(model, data)
 ...
 * @param {Function} done callback
 */
ESConnector.prototype.count = function count(modelName, where, done) {
    var self = this;
...
ESConnector.prototype.find = function find(modelName, id, done) {
    ...
    self.db.get(_.defaults({
        ...
        function (response) {
            done(null, self.dataSourceToModel(modelName, response, idName));
        }, function (err) {
            if (err.status == 404) {
                return done()
            }
...
//custom method
ESConnector.prototype.search = function (query, callback) {
    this.db.search(query, callback);
}

ESConnector.prototype.get = function (query, callback) {
    this.db.get(query, callback);
}

ESConnector.prototype.bulk = function (query, callback) {
    this.db.bulk(query, callback);
}

/**
 * @deprecated use for old project
 * @param query
 * @param callback
 */
ESConnector.prototype.update = function (query, callback) {
    this.db.update(query, callback);
}

ESConnector.prototype.index = function (query, callback) {
    this.db.index(query, callback);
}

ESConnector.prototype.delete = function (query, callback) {
    this.db.delete(query, callback);
}

ESConnector.prototype.scroll = function (query, callback) {
    this.db.scroll(query, callback);
}
